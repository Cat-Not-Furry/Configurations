...
class LauncherAppItem(widgets.Button):
    ...
    def __sync_menu(self) -> None:
        self._menu.model = IgnisMenuModel(
            IgnisMenuItem(label="Abrir", on_activate=lambda x: self.launch()),
            IgnisMenuSeparator(),
            *(
                IgnisMenuItem(
                    label=i.name,
            ...
            IgnisMenuSeparator(),
            IgnisMenuItem(label="Fijar", on_activate=lambda x: self._application.pin())
            if not self._application.is_pinned
            else IgnisMenuItem(
                label="Desfijar", on_activate=lambda x: self._application.unpin()
            ),
        )
