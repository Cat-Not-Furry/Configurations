        ...
        **kwargs,
        ...
        if menu:
            self._arrow_icon_widget = widgets.Icon(
                icon_name="go-next-symbolic", # Icono por defecto (cerrado)
                halign="end",
                hexpand=True,
                pixel_size=20, # Ajusta el tamaño si es necesario
            )
    ...
    def _update_arrow_icon_state(self, menu, _): # Renombrado
        if self._arrow_icon_widget: # Asegurarse de que el icono existe
            if menu.reveal_child:
                self._arrow_icon_widget.set_icon_name("go-down-symbolic") # V
            else:
                self._arrow_icon_widget.set_icon_name("go-next-symbolic") # >
