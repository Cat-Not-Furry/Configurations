...
```bash
# Desde la raíz del clone (~/Games/configurations)
...
./hyprland/scripts/install-hypr.sh --binaries   # + /usr/local/bin

# Tema
./hyprland/hyperland/scripts/apply-theme.sh blue
```

Alternativa directa al script maestro:

```bash
./hyprland/hyperland/scripts/deploy-configs.sh --config-hypr
```

El script resuelve rutas **relativas al clone** (`hyprland/waybar/`, `shared/wofi/`, `shared/ignis/`, etc.).
